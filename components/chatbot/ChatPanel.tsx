...
import { Send, Settings, Bot, Trash2, FileDown, FileText, X, BarChart2, TrendingUp, MapPin, Radio, Tag, AlertTriangle } from 'lucide-react';
...
const WELCOME: ChatMsgType = {
  role: 'assistant',
  content: 'Hola! Sóc l\'assistent analític del SAC. Pots preguntar-me sobre les dades del període seleccionat: sentiments, barris, canals, tendències, alertes...\n\nTambé pots exportar les dades en PDF o CSV, i descarregar els gràfics com a imatge.\n\n⚙️ Configura la clau d\'OpenAI per a respostes d\'IA reals.',
};

const SUGGESTED_QUERIES = [
  { label: 'Barris', query: 'Quins barris tenen més missatges?', icon: MapPin },
  { label: 'Canals', query: 'Com es distribueixen els canals de comunicació?', icon: Radio },
  { label: 'Categories', query: 'Quines són les principals categories?', icon: Tag },
  { label: 'Sentiment', query: 'Quin és el sentiment mitjà dels ciutadans?', icon: TrendingUp },
...
export function ChatPanel({ onClose }: ChatPanelProps) {
  ...
  async function exportToPdf() {
    ...
    try {
      ...
      pdf.save(`conversa-sac-${dateStr}.pdf`);
    } finally {
      setExporting(false);
    }
  }
  ...
  function exportLastChartToCsv() {
    ...
    if (!lastChart) return;
    const header = `Nom,Valor`;
    const rows = lastChart.data.map(d => `${d.name},${d.value}`);
    downloadCsv([header, ...rows].join('\n'), `${lastChart.title.replace(/\s+/g, '-').toLowerCase()}.csv`);
  }
    ...
    <div className="flex flex-col h-full bg-card border-l border-card-line shadow-2xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-card-line bg-primary shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          {hasChart && (
            <button
              onClick={exportLastChartToCsv}
              title="Exportar últim gràfic a CSV"
      ...
      <div className="border-t border-card-line p-3 shrink-0">
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="Fes una pregunta sobre les dades..."
        ...
        <p className="text-[10px] text-muted-foreground-2 mt-1.5 text-center">
          Assistent analític • Dades en temps real del SAC
        </p>
      </div>
